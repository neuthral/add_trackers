################################
#
#   add trackers for torrents in transmission from a curated list
#   github.com/neuthral
#

#!/bin/bash

LIST=$2
HOST="localhost:9091"

usage () {
    printf '\n%s\n' "Automatically add trackers for torrents in transmission from a curated list"
	printf '\n' ""
	printf '\n%s\n\n' "Usage: $(basename "$0") {id} {list.txt} "

	transmission-remote "$HOST" --list

	exit
}

if [[ ! -f $1 ]]; then
	usage
fi

add_trackers () {
    torrent_hash=$1
    echo "adding trackers for $torrent_name"
	while read -r LINE; do
        echo "${LINE} ..."
        if transmission-remote "$HOST" --torrent "${torrent_hash}" -td "${LINE}" | grep -q 'success'; then
            echo " done."
        else
            echo " already added."
        fi
        sleep 0.1
    done < ${LIST}
}

id="$1"
hash="$(transmission-remote "$HOST" --torrent "$id" --info | grep '^  Hash: ' | awk '{ print $2 }')"
torrent_name="$(transmission-remote "$HOST" --torrent "$id" --info | grep '^  Name: ' | cut -c 9-)"
add_trackers "$hash"
